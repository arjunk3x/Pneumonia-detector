...
# 1. Work out which internal role this user is (from session)
def _current_internal_role() -> str:
    """
    Internal role label used for logic, based on st.session_state["user_role"].
    Falls back to "SDA" if missing.
    """
    r = (st.session_state.get("user_role") or "SDA").replace(" ", "")
    # Normalise some common values, expand if needed
    if r in ["SDA", "DataGuild", "DigitalGuild", "ETIDM"]:
        return r
    # Very light inference from email if needed
    e = st.session_state.get("user_email", "").lower()
    if "dataguild" in e:
        return "DataGuild"
    if "digital" in e:
...
def _current_stage_label_for_role() -> str:
    """
    Map internal role (SDA/DataGuild/…) to the UI stage label
    used in STAGE_KEYS / STAGES.
    """
    return {
        "SDA": "SDA",
...
# Acting stage for this user (what they are *deciding for*)
current_stage = _current_stage_label_for_role()

meta = STAGE_KEYS.get(current_stage, {})
existing_status = str(t_row.get(meta.get("status", ""), "Pending"))

# Header (BIG heading + grey status line)
st.markdown(
    f"""
    <div style="margin-bottom:10px;">
      <span style="font-size:1.8rem;font-weight:700;">
...
# If stage not configured in STAGE_KEYS, we stop after info message
if current_stage not in STAGE_KEYS:
    st.info("This stage has no configured actions.")
    role_can_act = False    # just for safety; we won’t use it below
else:
    # =========================
    # Permissions
    # =========================
    user_internal_role = _current_internal_role()          # "SDA", "DataGuild", etc.
    user_stage_label = _current_stage_label_for_role()     # "SDA", "Data Guild", etc.
    # User can act only on *their* stage
    ...
    if submitted:
        ...
        # 4. Correct stage progression logic (uses your new helper)
        flag_field = meta["flag"]
        decision_field = meta["decision_at"]

        nxt = _next_stage(current_stage, tracker_df.loc[mask].iloc[0]) \
            if new_status == "Approved" else None

        # ---------- APPROVED ----------
        if new_status == "Approved":
            # Write timestamp -> stage completed
            tracker_df.loc[mask, decision_field] = when
            ...
            if nxt:
                ...
                tracker_df.loc[mask, "Current Stage"] = nxt
                tracker_df.loc[mask, "Overall_status"] = "In progress"
                # Only next stage flag = True
                for stg, m in STAGE_KEYS.items():
                    tracker_df.loc[mask, m["flag"]] = (stg == nxt)
            else:
                # No next stage -> fully approved
                tracker_df.loc[mask, "Overall_status"] = "Approved"

        # ---------- REJECTED ----------
        elif new_status == "Rejected":
            # Clear decision timestamp (stage not completed successfully)
            tracker_df.loc[mask, decision_field] = ""
            tracker_df.loc[mask, "Overall_status"] = "Rejected"
            tracker_df.loc[mask, "Current Stage"] = current_stage
            # All stage flags off
