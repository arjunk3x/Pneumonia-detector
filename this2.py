...
if "Sanction_ID" in sanctions_df.columns:
    sanctions_df["Sanction_ID"] = sanctions_df["Sanction_ID"].astype(str)
if "Sanction_ID" in tracker_df.columns:
    tracker_df["Sanction_ID"] = tracker_df["Sanction_ID"].astype(str)

s_row = sanctions_df.loc[sanctions_df["Sanction_ID"] == sid]
...
# 🔹 NEW: version of this submission (used in the button guard)
...
if submitted:
    ...
    decision_key = f"decision_done_{sid}_{current_stage}_{submitted_version}"

    if st.session_state.get(decision_key, False):
        st.warning("This decision has already been saved for this submission.")
        st.stop()
